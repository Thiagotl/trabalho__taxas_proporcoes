...
# density function
dKW<-function(y, mu = 0.7, sigma = 0.5, tau = 0.5,log = FALSE)
{
...
pKW<-function(q, mu = 0.7, sigma = 0.5,tau =0.5, lower.tail = TRUE, log.p = FALSE){
  if (any(mu <= 0) | any(mu >= 1)) stop(paste("mu must be between 0 and 1", "\n", ""))
  if (any(sigma < 0))  stop(paste("sigma must be positive", "\n", "")) 
...
#------------------------------------------------------------------------------------------ #ok
# quantile function
qKW<-function(u,mu,sigma,a=0,b=1,tau=0.5)
{
  if (any(mu <= 0) | any(mu >= 1)) stop(paste("mu must be between 0 and 1", "\n", ""))
  if (any(sigma < 0))  stop(paste("sigma must be positive", "\n", "")) 
...
rKW<-function(n,mu,sigma,a=0,b=1, tau =0.5)
{
  u<- runif(n)
